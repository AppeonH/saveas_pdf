...
forward
global type w_menu_functions from window
end type
type tv_1 from treeview within w_menu_functions
end type
type dw_2 from datawindow within w_menu_functions
end type
type dw_1 from datawindow within w_menu_functions
end type
type mle_output from multilineedit within w_menu_functions
end type
...
type gb_toolbaritem from groupbox within w_menu_functions
end type
type gb_function from groupbox within w_menu_functions
end type
type gb_3 from groupbox within w_menu_functions
end type
end forward
...
string title = "MenuItem Attributes"
...
public subroutine wf_constructor_menu ()
public subroutine wf_delete_menu ()
...
this.ChangeMenu (m_menu_functions_main)
			...
			for li_col_num = 1 to li_col_count step 1
				 ...
				 choose case lower(ls_name)
					      ...
					      case 'text'
							...
							case 'default'
						   	 ls_col_count = string(this.MenuID.item[ll_level1].default)							
							    if upper(ls_col_count)='TRUE' then
...
for i = 1 to UpperBound(this.menuid.item)
	ls_temp = this.menuid.item[i].text
	ls_data = '1-' + string(i)
	tv_item.label = ls_temp
	tv_item.data  = ls_data
	tv_item.children = True
	ll_level2 = tv_1.InsertItemLast(ll_roothandle,tv_item)
	FOR k = 1 to UpperBound(this.menuid.Item[i].Item)
		ls_temp = this.menuid.Item[i].Item[k].Text
		tv_item.label = ls_temp
		ls_data = '2-' + string(i) + '-' + string(k)
		tv_item.data  = ls_data
		If this.MenuID = m_menu_functions_main and i =  2 Then
			tv_item.children = True
		else
			tv_item.children = False
		end if
		ll_level3 = tv_1.InsertItemLast(ll_level2,tv_item)	
		FOR z = 1 to UpperBound(this.menuid.Item[i].Item[k].Item)
			ls_temp = this.menuid.Item[i].Item[k].Item[z].Text
			tv_item.label = ls_temp
...
event constructor;wf_constructor_menu()
end event
